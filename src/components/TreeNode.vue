<template>
  <li class="tree__item" :id="node.id? node.id : null">
    <span
      ...
      v-html="toggleButton"
    ></span>
    <span v-else-if="node.children" class="tree__toggle-btn-indent">&nbsp;</span>
    <span
      class="tree__node"
    ...
    >
      ...
      <input
        ...
        @drop="disableDrop"
      >
      <span v-else-if="node.editing" v-show="node.name" @click="nameClicked">{{node.name}}</span>
    </span>
    <transition name="fade">
      <ul class="tree__sub-tree" v-show="node.expended && node.children">
        <tree-node v-for="child in node.children" :key="child.id" :node="child" :options="options"></tree-node>
      </ul>
    </transition>
  </li>
</template>
